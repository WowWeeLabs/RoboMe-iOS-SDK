...
typedef enum {
    ...
    kRobotIncoming_DetectionVoltageStart = 0xA0, // Command detection sensitivity greater voltage less sensitive
...
@property BOOL red,green,blue;
@end

@interface SensorStatus : NSObject
@property BOOL edge, chest_100cm, chest_50cm, chest_20cm;
@end

@interface GeneralStatus : NSObject
@property BOOL showMood, remoteMovement, appMode, heartBeat;
@end

@interface IRStatus : NSObject
@property BOOL remoteControl, edgeDetection, chestDetection;
@end

@interface RobotControlProtocal : NSObject

// Command enum to string convert
...
+ (BOOL)isBatteryStatus: (int)command;
+ (BOOL)isRemoteButton: (int)command;
+ (BOOL)isSensorStatus: (int)command;
+ (BOOL)isLEDStatus: (int)command;
+ (BOOL)isMoodValue: (int)command;
+ (BOOL)isDirectionGameLevel: (int)command;
+ (BOOL)isGeneralStatus: (int)command;
+ (BOOL)isIRStatus: (int)command;
+ (BOOL)isHandshakeStatus: (int)command;
+ (BOOL)isDetectionVoltage: (int)command;

// Conversion helpers
+ (NSString *)decimalToBinaryString: (int)decimal;
+ (NSString *)intTo2DigitHexString: (int)command;
+ (int)hexStringToInt: (NSString *)hexString;

// Debug Output helpers
+ (void)outputRobotCommands;
+ (void)outputIncomingRobotCommands;
